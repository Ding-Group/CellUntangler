...
from ..distributions.rotated_wrapped_normal import RotatedWrappedNormal
...
class RotatedWrappedNormalProcedure(SamplingProcedure[RotatedWrappedNormal, RotatedWrappedNormal]):
    ...
    def reparametrize(self, z_mean: Tensor, std: Tensor) -> Tuple[RotatedWrappedNormal, WrappedNormal]:
        ...
        mu_0 = self._manifold.mu_0(torch.Size([1, z_mean.shape[-1]]), device=z_mean.device)
        std_0 = torch.ones(1, std.shape[-1], device=z_mean.device)
        p_z = RotatedWrappedNormal(mu_0, std_0, manifold=self._manifold)
